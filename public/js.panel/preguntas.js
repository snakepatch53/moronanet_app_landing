...
const handleFunction = {
    new: function () {
        ...
        $form["pregunta_id"].value = 0;
        bootstrap_modalform.show();
    },
    edit: function ($register_id) {
        const register = uiFunction.database.find((el) => el["pregunta_id"] == $register_id);
        setValuesForm(register, $form);
        bootstrap_modalform.show();
    },
    delete: function (register_id) {
        $form["pregunta_id"].value = register_id;
        bootstrap_modalconfirm.show();
    },

    // gift functions
    giftTrButton: function (register_id) {
        $form_gift["pregunta_id"].value = register_id;
...
const crudFunction = {
    select: async function () {
        await fetch_query(new FormData($form), "pregunta", "select").then((res) => {
            if (res.response) {
    ...
    insertUpdate: function (form) {
        const action = $form["pregunta_id"].value == 0 ? "insert" : "update";
        fetch_query(new FormData(form), "pregunta", action).then((res) => {
            uiFunction.modalForm_hide();
            this.select();
        });
    },
    delete: function () {
        fetch_query(new FormData($form), "pregunta", "delete").then((res) => {
            uiFunction.modalForm_hide();
            this.select();
...
const uiFunction = {
    database: [],
    giftDatabase: [],
    getTr: function ({ pregunta_id, pregunta_name }) {
        return `
            <tr>
                <td class="d-none d-md-table-cell fw-bold">${pregunta_id}</td>
                <td class="text-center text-md-left">
                    <p>${pregunta_name}</p>
                </td>
                <td class="text-center">
                    <button class="btn btn-outline-primary" onclick="handleFunction.edit(${pregunta_id})">
                        <i class="fa-solid fa-pen-to-square"></i>
                    </button>
                    <button class="btn btn-outline-danger" onclick="handleFunction.delete(${pregunta_id})">
